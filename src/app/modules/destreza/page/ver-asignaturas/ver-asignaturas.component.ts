...
import {
  ...
  Destreza,
  Objective,
} from '../../interfaces/asignatura.interface';
...
export class VerAsignaturasComponent implements OnInit {
  ...
  ngOnInit(): void {
    this.destrezaService.allAsignaturas().subscribe({
      next: ({ courses }) => {
        this.asignaturas.courses = courses;
        this.asignaturas.courses.map((c) => {
          const objt = c.Objectives.filter((o) => o.CourseId == c.id);
          console.log(objt);
        });
      },
      error: (error) => {
  ...
  llenarObjetivosDestrezas(curso) {
    let asignaturaSeleccionada = curso;
    if (asignaturaSeleccionada !== '0') {
      const asignatura = this.asignaturas.courses.filter((asignatura) => {
        return asignatura.nameCourse === curso;
      });
  ...
  validarAsignatura(curso) {
    this.llenarObjetivosDestrezas(curso);
  }
  ...
  updateDestrezaObjetivo() {
    if (this.isDestreza) {
      this.destrezaService
        .updateDestreza(this.valueUpdate.id, {
          nameDestreza: this.valueInputUpdate,
        })
        .subscribe({
          next: (destrezaResponse) => {
            const newArrDestrezas = this.asignaturaSeleccionada!.Destrezas.map(
              (destreza) => {
      ...
      this.destrezaService
        .updateObjective(this.valueUpdate.id, {
          nameObjective: this.valueInputUpdate,
        })
        .subscribe({
          next: (objectiveResponse) => {
            const newArrObjectives =
              this.asignaturaSeleccionada!.Objectives.map((objective) => {
  ...
  deleteDestrezaObjetivo() {
    if (this.isDestreza) {
      this.destrezaService.deleteDestreza(this.valueDelete).subscribe({
        next: (destrezaResponse) => {
          const newArrDestrezas = this.asignaturaSeleccionada!.Destrezas.filter(
            (destreza) => {
      ...
      this.destrezaService.deleteObjective(this.valueDelete).subscribe({
        next: (objectiveResponse) => {
          const newArrObjectives =
            this.asignaturaSeleccionada!.Objectives.filter((objective) => {
